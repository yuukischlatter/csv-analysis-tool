...
function addHeader(doc, pageWidth, colors) {
  ...
  doc.text("Protokoll zur Geschwindigkeitsmessung", 20, 24);
  
  // Underline
  doc.setDrawColor(...colors.border);
  doc.setLineWidth(0.3);
  doc.line(20, 27, pageWidth - 82, 27);
  
  // Logo placeholder - you can add your base64 logo string here
  const logoBase64 = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAA ... YII=";
  
  try {
    if (logoBase64 !== "") {
      doc.addImage(logoBase64, 'PNG', pageWidth - 74, 8, 63, 26);
    } else {
    ...
    doc.text("schlatter", pageWidth - 45, 18);
    
    // Logo box border
    doc.setDrawColor(...colors.schlatterBlue);
    doc.setLineWidth(0.5);
    doc.rect(pageWidth - 50, 12, 40, 12);
  }
...
function addInfoBoxes(doc, testData, colors) {
  ...
  // Top two info boxes
  addInfoBox(doc, "Auftragsdaten", leftX, startY, boxWidth, boxHeight, [
    ["Auftrags-Nr.:", testData.auftragsNr || ""],
    ["Maschinentyp:", testData.maschinentyp || ""]
  ], colors);
  
  addInfoBox(doc, "Regelventil", rightX, startY, boxWidth, boxHeight, [
  ...
  // Bottom two info boxes
  addInfoBox(doc, "Prüfung", leftX, startY + boxHeight + 5, boxWidth, reducedBoxHeight, [
    ["Prüfer:", testData.pruefer || ""],
    ["Datum:", testData.datumFormatted || formatDateForPDF(testData.datum) || ""]
  ], colors);
  
  addInfoBox(doc, "Prüfbedingungen", rightX, startY + boxHeight + 5, boxWidth, reducedBoxHeight, [
    ["Ventilabgleich UEQ:", (testData.ventilOffsetNachKorrektur || "0.00") + " V"],
